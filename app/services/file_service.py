    ...
    async def save_upload(self, upload: UploadFile) -> StoredFile:
        ...
        if kind == "image":
            try:
                ...
                destination.unlink(missing_ok=True)
                raise FileReadError("Файл не является корректным изображением.") from exc
    ...
    def get_output_artifacts(self, file_id: str) -> dict[str, list[dict[str, str]]]:
        ...
        for artifact in sorted(self.settings.output_dir.glob(f"{file_id}__*"), reverse=True):
            record = {
                ...
                "storage_url": build_download_url(artifact.name, self.settings, view=True),
                "download_url": build_download_url(artifact.name, self.settings),
            }
