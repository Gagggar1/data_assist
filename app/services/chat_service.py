...
class ChatService:
    ...
    def get_conversation(self, conversation_id: str) -> dict[str, Any]:
        self.ensure_storage()
        conversation = self.chat_repository.get(conversation_id)
        if not conversation:
    ...
    def build_page_context(self, request: Any, conversation: dict[str, Any]) -> dict[str, Any]:
        ...
        if active_file_id:
            try:
                active_file = self.file_service.get_file(active_file_id)
                chart_suggestions = self.chart_service.suggest_charts(active_file)
            except FileServiceError:
                chart_suggestions = []
        ...
        return {
            ...
            "prompt_role": self.settings.prompt_role,
        }
    ...
    def _handle_local_prompt(
        ...
        if action == "help":
            ...
            return self._new_message(
                "assistant",
                "Для активного файла могу показать предпросмотр, собрать анализ, построить histogram, bar, line, pie или scatter, сделать DOCX-отчёт и сохранить markdown-сводку.",
            )
        ...
        filtered.append(
            {
                ...
                "storage_url": f"/files/{stored_file.file_id}",
            }
        )
        return filtered
    ...
    def _stored_file_attachment(self, stored_file: StoredFile) -> dict[str, Any]:
        ...
        payload.update(
            {
                "download_url": f"/files/{stored_file.file_id}",
                "storage_url": f"/files/{stored_file.file_id}",
                "preview_url": f"/files/{stored_file.file_id}",
            }
        )
        return payload
    ...
    def _detect_chart_type(self, normalized: str) -> str:
        if any(token in normalized for token in ("hist", "гист")):
            return "histogram"
        if any(token in normalized for token in ("scatter", "точеч", "коррел")):
            return "scatter"
        if any(token in normalized for token in ("pie", "круг", "кругов", "дол")):
            return "pie"
