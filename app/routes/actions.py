from __future__ import annotations

from fastapi import (
    APIRouter,
    Depends,
    Form,
    HTTPException,
    Query,
    Request,
    status,
)
from fastapi.responses import FileResponse

from app.core.config import get_settings
from app.core.deps import (
    get_analysis_service,
    get_chart_service,
    get_file_service,
    get_report_service,
    get_templates,
)
from app.core.security import verify_api_key, verify_download_token
from app.services.analysis_service import AnalysisService
from app.services.chart_service import ChartService
from app.services.file_service import FileReadError, FileService
from app.services.report_service import ReportService

router = APIRouter()
settings = get_settings()


def _panel_context(
    request: Request,
    file_service: FileService,
    analysis_service: AnalysisService,
    file_id: str,
    success_message: str | None = None,
    error_message: str | None = None,
):
    stored_file = file_service.get_file(file_id)
    preview_context = file_service.build_preview_context(stored_file)
    analysis = analysis_service.analyze(stored_file)
    artifacts = file_service.get_output_artifacts(file_id)
    return {
        "request": request,
        "preview": preview_context,
        "analysis": analysis,
        "artifacts": artifacts,
        "success_message": success_message,
        "error_message": error_message,
    }


@router.post("/actions/analyze/{file_id}")
async def analyze_action(
    request: Request,
    file_id: str,
    file_service: FileService = Depends(get_file_service),
    analysis_service: AnalysisService = Depends(get_analysis_service),
):
    verify_api_key(request)
    templates = get_templates()
    context = _panel_context(request, file_service, analysis_service, file_id, success_message="Анализ обновлен.")
    return templates.TemplateResponse("partials/result_panel.html", context)


@router.post("/actions/chart/{file_id}")
async def chart_action(
    request: Request,
    file_id: str,
    chart_type: str = Form(...),
    x_column: str | None = Form(default=None),
    y_column: str | None = Form(default=None),
    file_service: FileService = Depends(get_file_service),
    analysis_service: AnalysisService = Depends(get_analysis_service),
    chart_service: ChartService = Depends(get_chart_service),
):
    verify_api_key(request)
    templates = get_templates()
    try:
        stored_file = file_service.get_file(file_id)
        chart_service.generate_chart(stored_file, chart_type, x_column=x_column or None, y_column=y_column or None)
        context = _panel_context(
            request,
            file_service,
            analysis_service,
            file_id,
            success_message="График построен и сохранен.",
        )
    except FileReadError as exc:
        context = _panel_context(request, file_service, analysis_service, file_id, error_message=str(exc))
    return templates.TemplateResponse("partials/result_panel.html", context)


@router.post("/actions/report/{file_id}")
async def report_action(
    request: Request,
    file_id: str,
    file_service: FileService = Depends(get_file_service),
    analysis_service: AnalysisService = Depends(get_analysis_service),
    chart_service: ChartService = Depends(get_chart_service),
    report_service: ReportService = Depends(get_report_service),
):
    verify_api_key(request)
    templates = get_templates()
    stored_file = file_service.get_file(file_id)
    preview_context = file_service.build_preview_context(stored_file)
    analysis = analysis_service.analyze(stored_file)
    artifacts = file_service.get_output_artifacts(file_id)

    chart_records = artifacts["charts"]
    if not chart_records:
        chart_service.generate_default_charts(stored_file)
        artifacts = file_service.get_output_artifacts(file_id)

    report_service.generate_report(stored_file, analysis, chart_records[:3], preview_context)
    context = _panel_context(
        request,
        file_service,
        analysis_service,
        file_id,
        success_message="DOCX-отчет сформирован и готов к скачиванию.",
    )
    return templates.TemplateResponse("partials/result_panel.html", context)


@router.get("/download/{artifact_name}")
async def download_artifact(
    request: Request,
    artifact_name: str,
    token: str | None = Query(default=None),
):
    verify_api_key(request)
    candidate = settings.output_dir / artifact_name
    if not candidate.exists() or candidate.parent.resolve() != settings.output_dir.resolve():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Файл не найден.")

    if not token or not verify_download_token(token, artifact_name, settings):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Недействительная ссылка для скачивания.")

    return FileResponse(path=candidate, filename=candidate.name)


@router.get("/artifacts/{artifact_name}/view")
async def view_artifact(
    request: Request,
    artifact_name: str,
    token: str | None = Query(default=None),
):
    """Serve generated artifacts inline after validating the signed link."""
    verify_api_key(request)
    candidate = settings.output_dir / artifact_name
    if not candidate.exists() or candidate.parent.resolve() != settings.output_dir.resolve():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Файл не найден.")
    if not token or not verify_download_token(token, artifact_name, settings):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Недействительная ссылка для просмотра.")
    return FileResponse(path=candidate)


@router.get("/files/{file_id}")
async def view_uploaded_file(
    request: Request,
    file_id: str,
    file_service: FileService = Depends(get_file_service),
):
    """Serve an uploaded file through authentication instead of public storage."""
    verify_api_key(request)
    try:
        stored_file = file_service.get_file(file_id)
    except FileReadError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc

    if not stored_file.path.is_file() or stored_file.path.parent.resolve() != settings.upload_dir.resolve():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Файл не найден.")
    return FileResponse(path=stored_file.path, media_type=stored_file.content_type)
